...
bool TryKuhn (size_t v, std::unordered_map<size_t, int>& mt, std::unordered_map<size_t, bool>& used, Graph &graph) {
	if (used[v]) {
        return false;
    }
	used[v] = true;
...
std::vector<std::pair<size_t, size_t>> KunAlgorithm(Graph &graph, const std::vector<size_t>& vertices_first_part, const std::vector<size_t>& vertices_second_part) {
